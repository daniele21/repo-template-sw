...
- `AGENTS.md` is routing/invariants, `.engineering/commands.json` is project operation/execution routing, `design/*` is product-experience/brand routing when applicable, Skills are conditional procedures, docs are durable facts, scripts/CI are deterministic enforcement/execution.
...
1. Identify whether the change belongs to universal standard, operating contract, execution-capability contract, product-experience contract, copied template, optional profile, adoption/update procedure or validation tooling.
